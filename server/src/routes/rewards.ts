...
export function createRewardsRouter(pool: DatabasePool) {
  ...
  router.get("/me",async(req,res,next)=>{try{
    ...
    const [ranking,transactions,rules,badges,total]=await Promise.all([
      ...
      pool.query(`SELECT br.id,br.code,br.name,br.description,br.metric,br.threshold,br.badge_type AS "type",br.tone,br.icon,ub.awarded_at AS "awardedAt" FROM badge_rules br LEFT JOIN user_badges ub ON ub.badge_rule_id=br.id AND ub.user_id=$1 WHERE br.enabled=true ORDER BY br.created_at`,[req.currentUser!.id]),
...
export function createAdminRewardsRouter(pool: DatabasePool) {
  ...
  router.get("/",async(req,res,next)=>{try{
    ...
    const [ranking,rules,badges,recent]=await Promise.all([
      ...
      pool.query(`SELECT br.id,br.code,br.name,br.description,br.metric,br.threshold,br.badge_type AS "type",br.tone,br.icon,br.enabled,COUNT(ub.id)::int people FROM badge_rules br LEFT JOIN user_badges ub ON ub.badge_rule_id=br.id GROUP BY br.id ORDER BY br.created_at`),
