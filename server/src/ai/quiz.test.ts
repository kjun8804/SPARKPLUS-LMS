...
describe("YouTube AI quiz", () => {
  it("accepts only HTTPS YouTube URLs", () => {
    expect(isPublicYouTubeUrl("https://www.youtube.com/watch?v=abcDEF_1234")).toBe(true);
    expect(isPublicYouTubeUrl("https://youtu.be/abcDEF_1234")).toBe(true);
    expect(isPublicYouTubeUrl("https://youtu.be/WeHFmpDN_ks")).toBe(true);
    expect(isPublicYouTubeUrl("https://www.youtube.com/watch?v=-WeHFmpDN_ks")).toBe(false);
  ...
  it("parses one four-option structured quiz", async () => {
    ...
    await expect(generateQuizFromYouTube({ apiKey:"test-key", youtubeUrl:"https://youtu.be/abcDEF_1234", fetcher })).resolves.toEqual(quiz);
  });
  ...
  it("rejects malformed model output", async () => {
    ...
    await expect(generateQuizFromYouTube({ apiKey:"test-key", youtubeUrl:"https://youtu.be/abcDEF_1234", fetcher })).rejects.toThrow();
  });
});
